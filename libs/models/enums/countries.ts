...
export const zCountryCode = z.enum(countryCodes);
export type CountryCodes = z.infer<typeof zCountryCode>;

export const languagesList = Object.entries(languages).map(([code, info]) => ({
	code,
	name: info.name,
}));
const languagesCodes = Object.keys(languages) as (keyof TLanguages)[];
export const zLanguageCode = z.enum(languagesCodes);
export type LanguagesCodes = z.infer<typeof zLanguageCode>;
