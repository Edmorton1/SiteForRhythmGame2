...
export type KafkaSenderReturn<T extends BASE_FUNCTIONS> = {
...
export class KafkaSender {
	constructor(
		...
		private readonly kafkaWebServer: KafkaLoader,
	) {}

	private sendMessage = (data: KafkaRequest, topic: TopicsRequest): void => {
		if (!this.kafkaWebServer.producer) throw new Error('ПРОДЮСЕР НЕ ЗАГРУЖЕН');
		this.kafkaWebServer.producer.send({
			topic,
	...
	initSender = <T extends BASE_FUNCTIONS>(): KafkaSenderReturn<T> => {
		return {
			sendAndWait: <F extends Extract<keyof T, string>>(
				data: { func: F; message: T[F]['input'] },
				topic: TopicsRequest,
